...
contract HarmoniaInterface is AccessControl, ReentrancyGuard {
    ...
    function updateCommunityRewards(uint256 _communityId) public {
        Community storage community = communities[_communityId];
        uint256 communityTotalStakeIncludingDonations = community.totalStaked + community.totalDonations;
        if (communityTotalStakeIncludingDonations == 0) {
            return;
        }
        ...
        // Get total staked ETH from the routerManager
        uint256 totalStakedETH = routerManager.getTotalStakedETH();
        ...
        if (communityETHChange > 0) {
            ...
            uint256 stakersETH = (community.totalStaked * communityETH) / communityTotalStakeIncludingDonations;
            uint256 donationsETH = communityETH - stakersETH;
    ...
    function unstake(uint256 _communityId, uint256 _amount) external nonReentrant returns (uint256) {
        ...
        uint256 requestId = routerManager.requestWithdrawal(msg.sender, _amount);
    ...
    function claimUnstakedETH(uint256 requestId) external nonReentrant {
        ...
        uint256 amount = routerManager.claimWithdrawal(requestId);
    ...
    // Emergency withdraw without caring about ETH rewards. FOR EMERGENCY ONLY.
    function emergencyWithdraw(uint256 _communityId) external nonReentrant {
        Community storage community = communities[_communityId];
        UserInfo storage user = userInfo[msg.sender];
        UserStake storage stakeInfo = user.stakes[_communityId];
        require(community.exists, "Community does not exist");
        ...
        // Calculate donation weighted stake
        uint256 donationWeightedStake = (amount * stakeInfo.donationPercent) / PERCENTAGE_DENOMINATOR;

        // Adjust totalDonationWeightedStake
        community.totalDonationWeightedStake -= donationWeightedStake;

        // Update unified donation rate
        updateUnifiedDonationRate(_communityId);

        // Burn hETH from the user
        hEthToken.burn(msg.sender, amount);

        // Update totalUserETH
        totalUserETH -= amount;

        // Update community's total staked
        community.totalStaked -= amount;
        ...
        // Transfer ETH back to the user via routerManager (handled by the routerManager)
        routerManager.transferETH(msg.sender, amount); // Immediate transfer for emergency

        emit EmergencyWithdraw(msg.sender, amount, _communityId);
    }
